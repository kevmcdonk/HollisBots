...
namespace HollisBots.Controllers
{
    ...
    public class HubNewsMessagesController : ApiController
    {
        private readonly ILifetimeScope scope;
        
        public HubNewsMessagesController(ILifetimeScope scope)
        {
        ...
        public async Task<HttpResponseMessage> Post([FromBody]Activity activity, CancellationToken token)
        {
            ...
            {
                System.Console.WriteLine("Hit an error in news controller: " + exp.Message + ";;;" + exp.StackTrace);
                throw exp;
            }
        }
                ...
                using (var scope = DialogModule.BeginLifetimeScope(Conversation.Container, message))
                {
                    ...
                    if (conversationupdate.MembersAdded.Any())
                    {
                        ...
                        foreach (var newMember in conversationupdate.MembersAdded)
                        {
                            ...
                            {
                                reply.Text = $"Welcome to the news bot {newMember.Name}! ";
                            }
                            else
                            {
                                reply.Text = $"Welcome to the news bot {message.From.Name}";
                            }
                            reply.ReplyToId = message.Recipient.Id;
                            await client.Conversations.ReplyToActivityAsync(reply);
                        }
